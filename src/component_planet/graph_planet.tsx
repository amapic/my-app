...
export const Liste_planete2 = () => {
    ...
    return (
        <div style={{ transition: "all 0.5s" }} onClick={handleClick}>
            Text
        </div>
    )
}
...
export const Liste_planete = () => {
        ...
        const subscription_liste_planete = liste_planete.subscribe(
            liste => {
                ...
                if (planete_active && liste) {
                    setPlanete_active(planete_active)
                    setItems(liste[planete_active])
                }
            }
        );
        return () => {
    ...
    return (
        items ?
            <table className="no-gutters aaa" id={"table-liste-planete"} style={{ width: "100%", transition: "all 0.5s" }}>

                <thead >
                    <tr className={"table-liste-planete-head"}>
                        <th style={{ width: "30%" }} className={"noselect"}>Rayon</th>
                        <th style={{ width: "70%" }} className={"noselect"}>Distance à l'étoile</th>
                    </tr>
                </thead>
                <tbody>

                    {Object.keys(items).map((planet_index, i) => {

                        return (
                            <tr key={shortid.generate()} className='table-row'>
                                <td>
                                    {items[planet_index].radius}
                                </td>
                                <td>
                                    {items[planet_index].semi_major_axis}
                                </td>
                            </tr>
